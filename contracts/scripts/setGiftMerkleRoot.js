...
const PUBLICPUBLISHER_ADDRESS_KEY = process.env.PUBLISHER_ADDRESS
...
const { generateGiftRoot } = require("./merkleTree")
const contractAddress = process.env.CONTRACT_ADDRESS

const nftContract = new web3.eth.Contract(contract.abi, contractAddress)

async function setGiftMerkleRoot(root) {
  ...
  const tx = {
    ...
    'data': nftContract.methods.setGiftMerkleRoot(root).encodeABI()
  };
  ...
  signPromise
    .then((signedTx) => {
      ...
      console.log(`setGiftMerkleRoot is complete! Set root to ${root.toString('hex')}`);
    })
    .catch((err) => {
...
const merkleRoot = generateGiftRoot();
setGiftMerkleRoot(merkleRoot)
